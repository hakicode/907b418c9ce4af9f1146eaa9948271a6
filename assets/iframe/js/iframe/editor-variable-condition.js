...
function initComboBoxVariable(ebox, idx, idy, prefix) {
    ...
    switch (prefix) {
        case 'if': {
            options.dataSource = _variables;
            break;
        }
        case 'then': {
            options.dataSource = _variables.filter(function (f) { return !f.readonly; });
            break;
        }
        case 'else': {
            options.dataSource = _variables.filter(function (f) { return !f.readonly; });
...
function initMultiSelectVariable(ebox, idx, idy, prefix) {
    ...
    switch (prefix) {
        ...
        case 'then': {
            variables = _variables.filter(function (f) { return !f.readonly; });
            break;
        }
        case 'else': {
            variables = _variables.filter(function (f) { return !f.readonly; });
...
function initComboBoxValue(ebox, idx, idy, prefix) {
    ...
    if (assignData.valuetype == 'variable') {
        ...
        options.dataSource = _variables.filter(function (f) { return f.type == ___variable[idx][prefix][idy].type });
...
function initComboBoxSlide(ebox, idx, idy, prefix) {
    ...
    var options = {
        ...
        template: '<div class="media">'
            ...
            + '<span>#:data.name||data.code||data.id#</span>'
            + '</div>'
            + '</div>',
    };
...
function initComboBoxNode(ebox, idx, idy, prefix) {
    ...
    var options = {
        ...
        template: '<div class="media">'
            + '<div class="media-body">'
            + '<h5 class="media-heading">#:data.code||data.id#</h5>'
            + '<div>'
